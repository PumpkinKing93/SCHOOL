...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        NewF = self.featExtractor.getFeatures(state, action)
        # sum not max
        # why does it have an issue with too many variables in self? it works
        return sum(NewF[NF] * self.getWeights(NF) for NF in NewF)
        "*** YOUR CODE HERE ***"
    ...
    def update(self, state, action, nextState, reward):
        ...
        for f in self.weights:
            self.weights[f] += self.alpha * diff * NewF[f]
        "*** YOUR CODE HERE ***"
